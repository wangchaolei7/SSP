...
class CamVidBase:
    def __init__(self, data_folder, DATASET, data_cfg, split):
        ...
        if not os.path.isdir(self.root_images):
            raise FileNotFoundError(f"CamVid images root not found: {self.root_images}")
        if not os.path.isdir(self.root_labels):
            raise FileNotFoundError(f"CamVid labels root not found: {self.root_labels}")
        ...
        if root_images and root_labels:
            if self.split != "train":
                ...
                if split_images and split_labels:
                    ...
                    candidate_images = os.path.join(base_path, self.split, self.frame_folder) if base_path else None
                    candidate_labels = os.path.join(base_path, self.split, self.mask_folder) if base_path else None
            ...
            if not base_path:
                raise ValueError("camvid requires data_cfg.path or data_cfg.root_images/root_labels")
            root_images = os.path.join(base_path, self.split, self.frame_folder)
            root_labels = os.path.join(base_path, self.split, self.mask_folder)
    ...
    def _frame_dir(self, v_name):
        return os.path.join(self.root_images, v_name)

    def _mask_dir(self, v_name):
        return os.path.join(self.root_labels, v_name)
    ...
    def _scan_record(self, v_name):
        ...
        if not os.path.isdir(label_dir):
            if is_main_process():
                warnings.append(f"CamVid labels missing: {label_dir}")
            return [], set(), stats, warnings

        label_suffix_ext = self._label_suffix_ext(self.label_extension)
        label_files = [
            f for f in os.listdir(label_dir)
            if f.lower().endswith(label_suffix_ext.lower())
        ]
        label_files.sort()
        ...
        if not os.path.isdir(frame_dir):
            ...
            if stats["skipped"] and is_main_process():
                warnings.append(
                    f"CamVid images missing: {frame_dir} (skipped {stats['skipped']} labels)"
                )
            return [], set(), stats, warnings
        ...
        if missing_images and is_main_process():
            warnings.append(
                f"CamVid pairing issue in {self.split}/{v_name}: "
...
class CamVidImageDataset(Dataset, CamVidBase):
    def __init__(
        self,
        data_folder,
        video_indices,
        ...
        CamVidBase.__init__(self, data_folder, DATASET, data_cfg, split)
    ...
    def _log_scan_stats(self):
        if not is_main_process():
            return
        print(
            "CamVid {} scan: total_gt={}, paired={}, skipped={}".format(
                self.split,
        ...
        if self._val_samples_limited:
            print(f"CamVid {self.split} samples limited to {len(self.samples)} (val_max_samples)")
        if self._scan_warnings:
            for warning in self._scan_warnings[:5]:
                print(warning)
            if len(self._scan_warnings) > 5:
                print("CamVid pairing warnings truncated")
...
class CamVidImageInferenceDataset(Dataset, CamVidBase):
    def __init__(
        self,
        data_folder,
        video_indices,
...
class CamVidImageLogitsDataset(CamVidImageDataset):
    def __init__(
        self,
        data_folder,
        video_indices,
...
class CamVidVideoDataset(Dataset, CamVidBase):
    def __init__(
        self,
        data_folder,
        video_indices,
...
class CamVidVideoLogitsDataset(CamVidVideoDataset):
    def __init__(
        self,
        data_folder,
        video_indices,
